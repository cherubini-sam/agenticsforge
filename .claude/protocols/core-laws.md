...
| 40  | Branch Isolation + Traceability | Every tool call MUST be traceable to a specific Task ID in task.md. Phase 5 execution MUST occur on a dedicated git branch named `{operation}/{slug}` (conventional-commits prefix enum). The branch is created at Phase 4 Step 0 via `git checkout -b {operation}/{slug}` directly on the main checkout (HEAD switches to the operation branch for the duration of Phases 4–6). Direct mutation of either protected base branch — `main` or `master`, both guarded identically — outside `.claude/artifacts/` is FORBIDDEN — enforced at every tool call by `.claude/hooks/block-destructive.sh`, which reads HEAD dynamically. Agents NEVER touch `main`/`master` tips. The branch is pushed to `origin` on first commit (`git push -u origin {operation}/{slug}`), not on creation. At Phase 6 close, HEAD REMAINS on `{operation}/{slug}`; promotion to `main` is a human-only operation (merge, squash, or rebase at user discretion). Commits MUST match `^(feat\|fix\|refactor\|docs\|chore\|test\|perf\|ci\|build\|style): [A-Z].+\\.$`. Enforced by `.githooks/commit-msg` and `.claude/hooks/block-destructive.sh`.                                          |
